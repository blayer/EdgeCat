...
import com.edgecat.app.worker.DownloadWorker
...
class DefaultDownloadRepository(
        ...
        when (workInfo.state) {
          WorkInfo.State.ENQUEUED -> {
            ...
            firebaseAnalytics?.logEvent(
              EdgeCatEvent.MODEL_DOWNLOAD.id,
  ...
  private fun sendNotification(title: String, text: String, taskId: String, modelName: String) {
    ...
    val channelName = "EdgeCat download notification"
    ...
    // Download from global model manager. Open the global model manager screen.
      ...
      intent =
        Intent(Intent.ACTION_VIEW, "com.edgecat.app://global_model_manager".toUri())
      ...
      intent =
        Intent(
            Intent.ACTION_VIEW,
            "com.edgecat.app://model/$taskId/${modelName}".toUri(),
          )
          .apply { flags = Intent.FLAG_ACTIVITY_NEW_TASK }
    }
