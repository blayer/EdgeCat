...
import com.mobileclaw.app.memory.FakeMemoryDao
...
class MemoryIntegrationTest {
  ...
  fun `memory context injected into replan prompt`() = runBlocking {
    ...
    val results = mapOf(
      "step_1" to com.mobileclaw.app.orchestration.StepResult(
        stepId = "step_1",
        status = com.mobileclaw.app.orchestration.StepStatus.FAILED,
        error = "ActivityNotFoundException",
      )
    )
    val evaluation = com.mobileclaw.app.orchestration.EvaluationResult(
      goalAchieved = false,
  ...
  fun `past repairs injected into diagnostic prompt`() = runBlocking {
    ...
    val failedStep = com.mobileclaw.app.orchestration.PlanStep(
      id = "step_1",
