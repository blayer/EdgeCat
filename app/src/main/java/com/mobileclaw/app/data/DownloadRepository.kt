...
import com.mobileclaw.app.worker.DownloadWorker
...
class DefaultDownloadRepository(
        ...
        when (workInfo.state) {
          WorkInfo.State.ENQUEUED -> {
            ...
            firebaseAnalytics?.logEvent(
              MobileClawEvent.MODEL_DOWNLOAD.id,
  ...
  private fun sendNotification(title: String, text: String, taskId: String, modelName: String) {
    ...
    val channelName = "Mobile-Claw download notification"
    ...
    // Download from global model manager. Open the global model manager screen.
      ...
      intent =
        Intent(Intent.ACTION_VIEW, "com.mobileclaw.app://global_model_manager".toUri())
      ...
      intent =
        Intent(
            Intent.ACTION_VIEW,
            "com.mobileclaw.app://model/$taskId/${modelName}".toUri(),
          )
          .apply { flags = Intent.FLAG_ACTIVITY_NEW_TASK }
    }
